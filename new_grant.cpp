...
void grant::new_grantfunc(std::string college_name)
{
	...
	std::cout<<old_grant<<"      "<<calc_grant<<std::endl;
	/*if(calc_grant > old_grant) 
	{
		std::cout<<"Accepted extra grant ammount = Rs."<<old_grant<<std::endl;
		grant_edit(old_grant, ob.college_name, 1);
	}
	else
	{
		std::cout<<"Accepted extra grant ammount = Rs."<<calc_grant<<" as per criterias set."<<std::endl;
